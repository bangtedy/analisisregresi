...
pjm %>% 
  kbl(caption = "Penjualan Motor (y) dan Pertumbuhan Pendapatan (x) Tahun 2001-2019") %>%
  kable_paper("hover", full_width=F)
```
...
Diagram pencar bisa menunjukkan secara kasar apakah hubungan antar variabel dapat dianggap linier atau tidak. Jika nilai $y$ cenderung meningkat atau menurun secara garis lurus ketika nilai $x$ meningkat, dan jika perpencaran pasangan titik-titik ($x,y$) berada disekitar garis lurus, maka kita dapat menjelaskan hubungan antara $y$ dan $x$ dengan menggunakan model regresi linier. Selanjutnya kita coba menarik garis lurus yang paling banyak "mendekati" titik-titik data pengamatan yang tersebar. 
...
Titik-titik pengamatan ada yang tepat digaris, ada juga yang di atas atau di bawah garis. Hal ini menunjukkan adanya variasi pada penjualan motor yang tidak berhubungan dengan tingkat pendapatan. Dengan kata lain terdapat faktor-faktor lain selain pendapatan yang juga mempengaruhi tingkat penjualan tetapi tidak kita masukkan ke dalam model.
