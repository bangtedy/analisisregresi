...
Analisis regresi adalah salah satu teknik statistik yang paling luas dipakai [@agresti2015], [@frees2010] dan [@lee2013].
...
```{r diagram-pencar, echo=FALSE, warning=FALSE, message=FALSE, fig.cap={"Diagram Pencar Penjualan Motor (y) sebagai Fungsi dari Pendapatan per Kapita (x)"}}
...
Selanjutnya kita tambahkan sebuah garis lurus yang paling banyak "mendekati" titik-titik data pengamatan yang tersebar. Sebuah garis lurus yang paling pas (*fit*) pada Gambar 1.2 menunjukkan bahwa proporsionalitas ini ternyata tidak ketat. Garis lurus yang kita tambahkan tidak menyinggung seluruh data yang tersebar. Dengan kata lain data sampel tidak bisa seluruhnya tepat berada pada garis yang dibuat. Titik-titik pengamatan ada yang tepat digaris, ada juga yang di atas atau di bawah garis. 

Hal ini menunjukkan adanya **variasi** pada penjualan motor yang tidak berhubungan dengan tingkat pendapatan. Atau terdapat faktor-faktor lain selain pendapatan yang juga mempengaruhi tingkat penjualan tetapi tidak tercakup di dalam model.


```{r garis-regresi, echo=FALSE, warning=FALSE, message= FALSE, fig.cap={"Garis Regresi Penjualan Motor (y) sebagai Fungsi dari Pendapatan per Kapita (x)"}}
ggplot(data = PJMotor, aes(x,y)) +
  geom_point() + theme_bw() + 
...
Persamaan umum garis regresi yang memodelkan hubungan antara $y$ dan $x$ adalah:
...
Persamaan ini menyatakan bahwa $y$ sebagai variabel respons adalah fungsi linier dari $x$, variabel prediktor. $\beta_0$ adalah konstanta yang menunjukkan nilai dari $y$ jika $x$ sama dengan nol. $\beta_1$ adalah koefisien kemiringan garis yang menunjukkan berapa banyak $y$ akan berubah jika $x$ meningkat sebanyak satu unit.

Tujuan utama kita dalam analisis regresi adalah mendapatkan nilai dugaan
$\hat \beta_0$ dan $\hat \beta_1$ terbaik sehingga rata-rata jarak vertikal untuk setiap titik data pengamatan dan nilai dugaannya secara kolektif paling
...
kita kumpulkan sehingga variasi yang tidak dapat dijelaskan oleh model minimum.

Salah satu cara untuk mendapatkan garis yang paling pas (*line of best fit*) yang sangat populerini adalah melalui suatu prosedur formal yang disebut *metode kuadrat terkecil/ordinary least squares* (OLS). 

```{r metode-ols, echo=FALSE, warning=FALSE, message=FALSE, fig.cap={"Mencari Garis Regresi dengan Metode Kuadrat Terkecil"}}
library("ggpmisc")
library("broom")
mpm <- lm(y~x)
...
Di dalam regresi sederhana, karena kita hanya menggunakan satu variabel prediktor, faktor lain selain pendapatan yang menyebabkan variasi pada penjualan
motor kita masukkan sebagai faktor acak (*random factor*). Faktor acak ini secara visual ditunjukkan oleh panjang garis vertikal (warna merah putus-putus) antara titik data pengamatan dengan garis regresi atau nilai dugaan variabel respons (garis biru).
...
$\epsilon_i$ dimasukkan ke dalam persamaan untuk mengakomodasi variasi dalam $y$ yang tidak dapat dijelaskan oleh variabel $x$. Faktor acak adalah istilah
statistik yang mewakili fluktuasi acak, kesalahan pengukuran, atau
dampak dari faktor-faktor diluar kendali peneliti [@faraway2016a] sehingga menyebabkan nilai dugaan tidak sama dengan nilai aktual (*error*). Jadi
...
Dalam banyak hal, jumlah variabel respons yang kita analisis lebih dari satu atau $p>1$, sehingga persamaan umum regresinya menjadi:
