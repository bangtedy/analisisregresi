...
serangga-serangga kecil) yang secara pasif terbawa oleh udara. Astronomi
...
pikiran manusia. Geologi mempelajari bentuk dan komposisi bumi.
...
$x_1, x_2, ... , x_p$, dimana p adalah jumlah prediktor. Jika $p=1$,
modelnya disebut regresi sederhana. Tetapi jika $p>1$, disebut regresi
berganda.
...
(prediktor/eksplanatori/independen). Jika memungkinkan, di dalam
analisis regresi kita mungkin juga ingin menyimpulkan apakah terdapat
hubungan sebab-akibat: mengetahui pengaruh dari variabel prediktor
...
6.  Hubungan antara desain pekerjaan (x) dengan perilaku karyawan di
    ...
    analisis sendiri seperti [analisis
    faktor](https:bangtedy.github.io/analisisfaktor). Topik ini diluar
    cakupan dari buku ini.
...
faktor yang mempengaruhi pendapatan seseorang. Hipotesis kita semakin
tinggi tingkat pendidikan seseorang akan semakin tinggi pula tingkat
pendapatannya. Jadi terdapat hubungan positif antara pendapatan dengan
pendidikan.
...
Selanjutnya kita mengambil sampel secara acak dari populasi,
pasangan-pasangan data x dan y sebanyak n
$(x_1, y_1), (x_2, y_2), ... (x_n,y_n)$ atau kita tuliskan:
...
Ketika kita mencoba menjelaskan variabel $y$ dengan menggunakan variabel
$x$ ini, kita menggunakan hubungan statistik: sebuah hubungan yang tidak
eksak. Jika data yang telah kita kumpulkan, dibuatkan visualisasi
diagram pencarnya, kurang lebih kita akan mendapatkan seperti Gambar
1.1. Gambar ini secara jelas menunjukkan terdapat hubungan positif
antara pendapatan dengan tingkat pendidikan: semakin tinggi pendidikan
...
set.seed(11111) # gunakan seed yang sama jika ingin mendapatkan hasil yang sama
x = runif(31,0,10)
x[28] = 8.5
y = 10 + 2*x + rnorm(31)*3
#
...
ketidaksempurnaan hubungan antara pendapatan dengan pendidikan. Tampak
bahwa data sampel kita tidak seluruhnya tepat berada pada garis regresi.
Titik-titik pengamatan ada yang tepat digaris, ada juga yang di atas
atau di bawah garis regresi. Hal ini menunjukkan bahwa ada variasi
pendapatan yang tidak berhubungan dengan tingkat pendidikan. Artinya ada
faktor lain selain pendidikan yang juga mempengaruhi tingkat pendapatan
seseorang.
...
$\hat \beta_0$ dan $\hat \beta_1$ terbaik yang menghasilkan jumlah
kesalahan kuadrat terkecil. Dengan kata lain kita ingin menarik garis
dalam bidang x−y yang paling pas dengan persebaran titik-titik data
sampel yang telah kita kumpulkan. Garis ini disebut sebagai garis
regresi $\hat y_i = \hat \beta_0 + \hat \beta_1x_i$ yang menunjukkan
nilai dugaan y (disimbolkan dengan y tudung $\hat y$) untuk pengamatan
ke-i di dalam sampel.
