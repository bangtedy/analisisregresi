...
Langkah awal untuk melihat apakah benar-benar terdapat hubungan antara
pendapatan dengan penjualan motor adalah membuat diagram pencar
(*scatter plot*), seperti yang ditunjukkan oleh Gambar 1.1. Gambar ini
menunjukkan terdapat hubungan positif antara pertumbuhan pendapatan per
kapita dengan tingkat penjualan motor: semakin tinggi pertumbuhan
pendapatan per kapita semakin tinggi tingkat penjualan motor.
...
Selanjutnya dari diagram pencar ini, kita buat garis lurus yang paling
banyak "menyentuh" data yang tersebar seperti yang ditunjukkan oleh
Gambar 1.2. Tentu saja garis yang kita buat tidak bisa menyinggung
seluruh data yang tersebar atau data sampel tidak bisa seluruhnya tepat
berada pada garis yang dibuat.

Titik-titik pengamatan mungkin ada yang tepat digaris, ada juga yang di
...
```{r garis-regresi, echo=FALSE, fig.cap="Estimasi Persamaan Linier"}
PJMotor <- read.delim("PJMotor.txt")
plot(x, y)
...
ditunjukkan oleh panjang garis vertikal (warna kuning putus-putus)
...
komponen fungsional yaitu $\beta_0 + \beta_1$ dan komponen
stokastik/acak yaitu $\epsilon$. Kombinasi kedua komponen ini menjadikan
persamaan regresi sebagai model yang tidak eksak. Kita hanya
mengasumsikan bahwa di dalam cakupan variabel yang dianalisis, terdapat
tendensi variabel respons $y$ untuk bervariasi secara sistematis dengan
...
[@box1979] *"all models are wrong, but some are useful."* Seperti
