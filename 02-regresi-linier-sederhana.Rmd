...
Hasil regresi menunjukkan dugaan $\hat \beta$. Intersep garis regresinya adalah $\hat \beta_0$ = -1.210 (SE= 2.088), p-value = 0.57 (tidak signifikan). Sedangkan $\hat \beta_1$ = 1.733 (SE = 0.525) dan p-valuenya (menguji hipotesis nol bahwa $\beta_1$ = 0) kecil, konsisten dengan bukti persebaran data yang cenderung linier. 
...
Kolom-kolom pada tabe diatas menunjukkan
...
Kode berikut memplot residuals error (warna kuning) antara nilai pengamatan dengan nilai dugaan. Setiap garis vertikal warna kuning menunjukkan nilai kesalahan/residu antara angka penjualan motor aktual dengan nilai dugaannya.
...
ggplot(uji.diagnostik, aes(x, y)) +
  ...
  geom_segment(aes(xend = x, yend = .fitted), color = "gold", size = 0.3)
```
