...
Pada diagonal, kita melihat distribusi data $y$ dan $x$: data kedua variabel menceng ke kiri (*skewed left*). Di atas diagonal ditunjukkan nilai korelasi $y$ dan $x$ yaitu sebesar=0.625. Dibawah diagonal menunjukkan diagram pencar pasangan data.
...
Kode berikut memplot residuals error (warna merah) yaitu selisih antara nilai pengamatan dengan nilai dugaan. Setiap garis vertikal warna merah menunjukkan nilai kesalahan/residu antara data penjualan motor aktual dengan nilai dugaannya.

```{r plot-residu-mpm, echo=TRUE, warning=FALSE, fig.cap={"Plot Residual Error"}, fig.align="center"}
library(ggplot2)
my.formula <- y ~ x
...
Diagram pencar, garis dan persamaan regresi dapat ditampilkan bersama-sama dengan perintah berikut:

```{r plot-mpm, echo=TRUE, warning=FALSE, fig.cap={"Diagram Pencar, Garis dan Persamaan Regresi"},fig.align="center"}
library(ggpmisc)
PJMotor <- as.data.frame(PJMotor)
my.formula <- y ~ x
